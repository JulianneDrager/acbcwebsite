...
const Admin = () => {
  ...
  const [customerId, setCustomerId] = useState(null); // Add this line if you don't have it already

  useEffect(() => {
    const fetchPayments = async () => {
      const { data: payments } = await axios.get(
        `https://acbcwebsite.onrender.com/api/payments?customerId=${customerId}`
      );
  ...
  useEffect(() => {
    ...
    const filteredPayments = listPayments.filter(
          ...
          {filtered.map((result, index) => (
            <Row key={index}>
              ...
              <aside style={{ padding: "0 1rem" }}>
                ...
                <div style={{ display: "flex" }}>
                  <Col xs={5}>Name</Col>
                  <Col xs={5}>{result.companyName}</Col>
                </div>
                {/* <div style={{ display: "flex" }}>
                  <Col xs={5}>Address</Col>
                  <Col xs={5}>
                    {result.address ? result.address.addressLine1 : "N/A"}
                  </Col>
                </div>
                ...
                <div style={{ display: "flex" }}>
                  ...
                  <Col xs={5}>
                    {result.address
                      ? `${result.address.locality}, ${result.address.postalCode}`
                      : "N/A"}
                  </Col>
                </div> */}
                ...
                <div>
                  {filteredPayments.map((payment) => (
                    <div key={payment.id}>
                      <p>
                        Billing Address:{" "}
                        {payment.billing_address.address_line_1}
                      </p>
                      <p>City: {payment.billing_address.locality}</p>
                      <p>Postal Code: {payment.billing_address.postal_code}</p>
                    </div>
                  ))}
                </div>
              </aside>
